...
% read the Normal labels information
[filenames,~, labels] = textread('data_Norm_labels.txt','%s%s%s');

if ~exist(fullfile('./mias_preprocess_Part1/original','Normal'),'dir')
    mkdir(fullfile('./mias_preprocess_Part1/original','Normal')) 
end  
...
%      ADAPTHISTEQ
    ...
    points = ginput(1);  %获取当前点坐标(裁切图的左上角点的位置)
    coor_x = points(1,1);% obtain the top-left x location of cropped box
    coor_y = points(1,2); % obtain the top-left y location of cropped box    
    ...
    croppedImg_foreg = imresize(croppedImg_foreg, [crop_h, crop_w]);
     ...
     if nn >=2 
         ...
         if  flag_same ==1
             ...
             imgname = ['Normal_', curFileName, '_', num2str(cur_repeat)];
             imwrite(croppedImg_foreg,['./mias_preprocess_Part1/original/Normal/',imgname,'.png']);     
         else
             imgname = ['Normal_', curFileName];
             imwrite(croppedImg_foreg,['./mias_preprocess_Part1/original/Normal/',imgname,'.png']);
             cur_repeat = 1;
         end  
     % when nn=1   
