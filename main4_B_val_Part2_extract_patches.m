...
sub_category = fullfile('./mias_preprocess_Part1/trainvaltest/val/Benign/*.png'); % obtan
...
for nn=1:sub_len
    ...
    Imgpath = fullfile('./mias_preprocess_Part1/trainvaltest/val/Benign', Img_sub_catgory(nn).name);    
    % read the image
     ...
     while patch_count < totalNum_eachPatch
        ...
        % save the small patch        
         if ~exist(fullfile(ForCNN_Path, 'val/Benign/'),'dir')
                 mkdir(fullfile(ForCNN_Path,'val/Benign/')) 
         end     
        new_filepath = [ForCNN_Path,'/val/Benign/', filename_only,'_',num2str(patch_count,'%04d'),'.png'];    
