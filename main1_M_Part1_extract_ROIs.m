...
% read the Malignant labels information
[filenames,~,~, labels,x,y,radius] = textread('data_M_labels.txt','%s%s%s%s%d%d%d');
if ~exist(fullfile('./mias_preprocess_Part1/original','Malignant'),'dir')
    mkdir(fullfile('./mias_preprocess_Part1/original','Malignant')) 
end  
    ...
    croppedImg_foreg = imresize(croppedImg_foreg, [crop_h, crop_w]);
     ...
     if nn >=2 
         ...
         if  flag_same ==1
             ...
             imgname = ['Malignant_', curFileName, '_', num2str(cur_repeat)];
             imwrite(croppedImg_foreg,['./mias_preprocess_Part1/original/Malignant/',imgname,'.png']);     
         else
             imgname = ['Malignant_', curFileName];
             imwrite(croppedImg_foreg,['./mias_preprocess_Part1/original/Malignant/',imgname,'.png']);
             cur_repeat = 1;
         end  
     % when nn=1   
