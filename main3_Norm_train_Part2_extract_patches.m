...
sub_category = fullfile('./mias_preprocess_Part1/trainvaltest/train/Normal/*.png'); % obtan
Img_sub_catgory = dir(sub_category);
row= 120;
col = 120;
patch_height = 72;
pathch_width = 72;
steps_row = patch_height/2;
steps_col = pathch_width/2;
is_Visulize = 0; % 1:true, 0:false   
threshold_bg = 0; 

% the number for extracting small patch for each bigger patch
totalNum_eachPatch = 250*2;

%%%%%%%%%%%%%%%%%%%%% Step 2: To extract 
sprintf(['Start to deal with the sub category: Normal!\n'])    
sub_len = length(Img_sub_catgory); %
for nn=1:sub_len
    sprintf(['Start to deal with the No.%d image!\n'],nn)  
    Imgpath = fullfile('./mias_preprocess_Part1/trainvaltest/train/Normal', Img_sub_catgory(nn).name);    
    % read the image
     ...
     while patch_count < totalNum_eachPatch
        ...
        % save the small patch
         if ~exist(fullfile(ForCNN_Path,  'train/Normal/'),'dir')
                 mkdir(fullfile(ForCNN_Path, 'train/Normal/')) 
         end     
        new_filepath = [ForCNN_Path, '/train/Normal/', filename_only,'_',num2str(patch_count,'%04d'),'.png'];    
