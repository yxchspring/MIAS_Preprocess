...
% ForCNN_Path: You should set the save location for the extracting ROI patches
ForCNN_Path = 'D:\Data\Image\Biomedicine\integrated\MIAS_Patches\MIAS_B_M_Norm_Preprocess\MIAS_BMN_ForCNN_More'; 
sub_category = fullfile('./mias_preprocess_Part1/trainvaltest/train/Benign/*.png'); % obtan
...
%%%%%%%%%%%%%%%%%%%%% Step 2: To extract 
sprintf(['Start to deal with the sub category: Benign!\n'])    
sub_len = length(Img_sub_catgory); %
for nn=1:sub_len
    sprintf(['Start to deal with the No.%d image!\n'],nn)  
    Imgpath = fullfile('./mias_preprocess_Part1/trainvaltest/train/Benign', Img_sub_catgory(nn).name);    
    % read the image
     ...
     while patch_count < totalNum_eachPatch
        ...
        % save the small patch        
         if ~exist(fullfile(ForCNN_Path, 'train/Benign/'),'dir')
                 mkdir(fullfile(ForCNN_Path,'train/Benign/')) 
         end     
        new_filepath = [ForCNN_Path,'/train/Benign/', filename_only,'_',num2str(patch_count,'%04d'),'.png'];    
